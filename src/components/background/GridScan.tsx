...
type GridScanProps = {
    ...
    linesColor?: string;
    linesOpacity?: number; // Added new prop
...
uniform float uLinesOpacity; // Added uniform
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  vec3 gridCol = uLinesColor * lineVis * fade * uLinesOpacity; // Apply opacity here
  ...
  float alpha = clamp(max(lineVis * uLinesOpacity, combinedPulse), 0.0, 1.0); // Adjust alpha calculation
...
export const GridScan: React.FC<GridScanProps> = ({
    ...
    lineThickness = 1,
    linesColor = '#392e4e',
    linesOpacity = 1.0, // Default to 1.0
    scanColor = '#FF9FFC',
    scanOpacity = 0.4,
    gridScale = 0.1,
    lineStyle = 'solid',
    ...
    useEffect(() => {
        ...
        // Use window listener for global interaction
        ...
        // el.addEventListener('mouseenter', onEnter); // Might not need these if global
        // if (scanOnClick) el.addEventListener('click', onClick); // Maybe global click?
        // el.addEventListener('mouseleave', onLeave);

        // For now, keep original behavior for enter/leave but switch move to window
        // Actually, if we use window.mousemove, we don't really 'leave' unless we leave the window
        // So let's just stick to window mousemove for interaction logic.
        // If we want "leave" behavior when mouse leaves the app window:
    ...
    useEffect(() => {
        ...
        const uniforms = {
            ...
            uLinesOpacity: { value: linesOpacity }, // Added here
    ...
    useEffect(() => {
        ...
        if (m) {
            ...
            u.uLinesOpacity.value = linesOpacity; // Update uniform
